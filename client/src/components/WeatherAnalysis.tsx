...
interface WeatherData {
  currentWeather: { temperature: number; humidity: number; rainfall: number; windSpeed: number; condition: string; };
  forecast: { day: string; temp: number; rain: number; condition: string; }[];
...
const fetchWeatherAnalysis = async (location: string, crop: string): Promise<WeatherData> => {
  ...
  if (!response.ok) throw new Error("Failed to fetch weather analysis from the server.");
